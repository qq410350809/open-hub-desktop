...
onMounted(async () => {
  if (!isTauri) return;
  unlistenProgress = await listen<SyncSitesProgress>("chrome-account-sync-progress", (event) => {
      ...
      >
        <header class="link-dialog-header">
          ...
          <button
            ...
            :disabled="Boolean(store.chromeBrowserSyncingProfileId.value)"
            @click="close"
            v-html="icons.close"
          />
        </header>
        ...
        <div class="chrome-session-list">
          ...
          <template v-else>
            ...
            <section
              ...
              aria-label="Chrome 账号同步日志"
              aria-live="polite"
            >
              <header>
                <div>
                  <strong>Chrome 同步日志</strong>
                  <span
                    :class="{
                      'is-error': Boolean(store.chromeBrowserSyncError.value),
                      'is-complete': !store.chromeBrowserSyncingProfileId.value && !store.chromeBrowserSyncError.value,
                    }"
            ...
            <div class="chrome-session-actions">
              ...
              <button
                ...
                title="复制会话"
                :disabled="store.chromeSessionCopyingProfileId.value === session.profileId || Boolean(store.chromeBrowserSyncingProfileId.value)"
