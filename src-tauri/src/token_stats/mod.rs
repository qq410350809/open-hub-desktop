...
mod tests {
    ...
    fn reads_catpawai_usage_and_resolves_numeric_model_ids() {
        ...
        assert_eq!(bucket.input_tokens, 1000);
...
mod activity_tests {
    ...
    fn claude_user_is_human_filters_non_user_input() {
        ...
        assert!(!crate::token_collector::claude_user_is_human(&tool_only));

        let text = json!([{"type": "text", "text": "hello"}]);
        assert!(crate::token_collector::claude_user_is_human(&text));

        let plain = json!("hello");
        assert!(crate::token_collector::claude_user_is_human(&plain));

        let interrupted =
            json!([{"type": "text", "text": "[Request interrupted by user for tool use]"}]);
        assert!(!crate::token_collector::claude_user_is_human(&interrupted));

        let stdout =
            json!([{"type": "text", "text": "<local-command-stdout>ok</local-command-stdout>"}]);
        assert!(!crate::token_collector::claude_user_is_human(&stdout));
        let cmd = json!("<command-name>/compact</command-name>");
        assert!(crate::token_collector::claude_user_is_human(&cmd));
    }
