...
mod tests {
    ...
    fn cline_parser_reads_tokens_and_cost() {
        let dir = std::env::temp_dir().join(format!("openhub_cline_test_{}", std::time::SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()));
    ...
    fn continue_parser_reads_tokens_and_messages() {
        let dir = std::env::temp_dir().join(format!("openhub_continue_test_{}", std::time::SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()));
        let _ = fs::create_dir_all(&dir);
    ...
    fn aider_parser_extracts_tokens_from_chat_history() {
        let dir = std::env::temp_dir().join(format!("openhub_aider_test_{}", std::time::SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()));
        let _ = fs::create_dir_all(&dir);
    ...
    fn goose_and_catpawai_parsers_parse_valid_records() {
        let dir = std::env::temp_dir().join(format!("openhub_misc_test_{}", std::time::SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_nanos()));
        let _ = fs::create_dir_all(&dir);
