...
use crate::token_stats::catpawai::collect_catpawai_activity_incremental;
    ...
    if kind == "response_item" {
        if use_response_item_users {
            ...
            if payload.get("type").and_then(JsonValue::as_str) == Some("message")
                ...
                && crate::token_collector::codex_user_message_is_human(payload)
            {
...
pub fn command_code_on_line(
    ...
    if role == "user" {
        ...
        if crate::token_collector::claude_user_is_human(content) {
...
pub fn dsh_on_line(
    ...
    match kind {
        "user/message" => {
            if crate::token_collector::dsh_user_is_human(value) {
                *last_user_hour = Some(hour.clone());
...
pub fn collect_request_health_snapshot(force: bool) -> Result<RequestHealthReport, String> {
    ...
    let kiro_root = crate::token_collector::kiro_v2_session_root(&home);
    if kiro_root.is_dir() {
