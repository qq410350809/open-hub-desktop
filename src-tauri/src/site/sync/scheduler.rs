...
fn load_recovery_targets(connection: &rusqlite::Connection) -> Result<Vec<RecoveryTarget>, String> {
    ...
    ) in rows
    {
        ...
        // 普通网络抖动保留缓存等下一轮直连重试。
        if !sync::requires_chrome_fallback(&sync_error) {
            continue;
        }
        if sync::browser_fallback_cooldown_remaining_ms(failed_at, fail_count) > 0 {
            continue;
        }
...
async fn run_auto_sync_round(app: &AppHandle) -> AutoSyncRoundSummary {
    ...
    let scan_result = sync::mark_sites_with_chrome_sessions(
        app.clone(),
        database.clone(),
        None,
        None,
    ...
    for target in &recovery_targets {
        ...
        match sync::sync_site_account_via_chrome_command(
            app.clone(),
            &database,
    ...
    for target in &model_targets {
        ...
        match catalog::auto_fetch_site_models_json(
            app,
            &database,
            base_url,
            target.site_id.clone(),
        ...
        .await
        {
            Ok(result) => {
                ...
                if !cleared_sites.contains(&target.site_id) {
                    let _ = catalog::clear_site_model_cache(&database, &target.site_id);
                ...
                match catalog::save_site_model_cache(
                    &database,
                    &target.site_id,
                    &account,
                    Some(&result),
