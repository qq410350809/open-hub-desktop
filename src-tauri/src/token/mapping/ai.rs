...
pub fn build_prompt(
    ...
    let standards_block = if standards.is_empty() {
        ...
        let lines = standards
            ...
            .join("\n");
        format!(
            "\n\n已人工批准的标准映射（仅当待判定条目与标准条目的版本号完全一致时才可沿用；\
             版本号不同属于不同模型，禁止沿用）：\n{lines}"
        )
    };
    format!(
        "你在执行本地 Token 统计的模型名称识别。输出仅作为人工审核建议，不能替代人工确认。\n\
         ...
         3. 版本号不同的模型是不同模型（如 3.6 与 3.7、5.2 与 5.3、opus 与 sonnet）；\
         即使标准映射里存在同族条目，版本不一致也禁止沿用，只能选待判定条目自身版本对应的候选；\n\
         4. rawModel 必须逐字复用待判定条目的原始名，每条至多输出一次；\n\
         5. confidence 是 0 到 1 的有限小数；reason 用一句不超过 40 字的中文，\
         写明具体依据（命中的候选名、别名或版本一致的标准条目），不要写“候选清单中有匹配”这类空话；\n\
         6. 不要把不同模型因为名称相似而强行合并。\n\
