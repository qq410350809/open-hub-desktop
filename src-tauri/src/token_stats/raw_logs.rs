...
use crate::token_stats::catpawai::catpawai_data_roots;
...
pub fn parse_claude_file(
    ...
    for line in text.lines() {
        ...
        if kind == "user" {
            ...
            if !crate::token_collector::claude_user_line_is_human(&value, &content) {
                continue;
            }
...
pub fn collect_local_agent_paths(home: &Path) -> LocalAgentPathsReport {
    ...
    {
        let mut entries = Vec::new();
        for p in crate::token_collector::collect_cursor_db_paths(home) {
    ...
    {
        let mut entries = Vec::new();
        for p in crate::token_collector::collect_windsurf_db_paths(home) {
            push_agent_path(&mut entries, "database", "SQLite state.vscdb", Some(&p));
        }
        let root = entries.first().and_then(|e| PathBuf::from(&e.path).parent().map(|p| p.to_path_buf()));
    ...
    {
        let mut entries = Vec::new();
        for p in crate::token_collector::collect_zed_source_files(home) {
    ...
    // Cline
    let cline_files = crate::token_collector::collect_cline_source_files(home);
    {
        let mut entries = Vec::new();
    ...
    {
        let mut entries = Vec::new();
        for p in crate::token_collector::collect_aider_source_files(home) {
